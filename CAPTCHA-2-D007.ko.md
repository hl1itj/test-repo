...
1. 가상적 로그인 페이지 만들기. 백그라운드 이미지 위에. a. ID(이메일) 입력창, b. 비밀번호 (입력하면 * 표시) 입력창, c. 50x160px 크기의 4-8자리의 숫자 임의의 CAPTCHA 이미지가 있으며, d, CAPTCHA 이미지에 대한 답 입력창, e. 백그라운드 이미지, CAPTCHA 이미지와는 다른 CAPTCHA와 같은 크기의 임의의 이미지. a,b,c,d,e 다섯 부분이 매번 접속 때마다 임의의 위치에 배치되어야함. c,e 의 이미지는 이미지 형식의 확장자를 가진 파일로 앞의 파일이름은 8글자의 random 생성된 스트링이어야 함. 이메일 주소 형식에 맞는 이메일과, 8자리 이상의 반드시 대소문자, 숫자, 특수문자 조합의 비밀번호가 들어오고, 제시된 CAPTCHA 숫자가 맞으면 브라우저에 "OK", 틀리면 'NOT OK"를 출력하는 홈페이지를 로컬 환경에 구성
...
1. 위 홈페이지 로그인에 사용할 이메일 주소 "name@subdomain.domain.com"의 "name"은 실제 존재하는 이름, "subdomain"과 "domain"은 실제 존재하는 단어를 사용. 이름은 '조선실록' 신문사 홈페이지에서 기자 이름들을 추출하여 사용. subdomain, domain 단어는 Unix(또는 MacOS, 기타 운영체제) dictionary에서 있는 것을 random으로 선택하여 사용
1. 자동 생성된 숫자 CAPTCHA 이미지 내의 숫자 가운데 같은 크기,폰트,기울기 숫자는 두 개까지만 허용됨
1. 보너스 문제의 차량 번호판은 "12가3456"와 같은 2자리+한글+4자리 형식의 번호판으로 제한. 번호판 이미지는 실물 번호판의 비율로 흰색, 초록색, 노랑색으로 생성하고 인위적으로 잡음을 섞고 (SNR 신호대 잡음비   , 마름모 모양의 사각형으로 변형한 것을 주로 사용하고 실제 번호판 이미지 100개 이상을 수집하여 학습에 사용.
...
```
1. MNIST Tutorial 구현 내용은 선배 또는 멘토에 의한 동료 평가로 확인
1. 여러 숫자 이미지 과제와, 숫자 CAPTCHA 생성 프로그램은 직접 작성하는 것이 과제이며, 결과 검증 모드로 실행할 때에서는 과제의 데이터 파일에 있는 실행 파일을 통해 생성된 00.png ~ 99.png 100개의 이미지를 이용하되, 이름을 이미지를 차례로 읽어 이름을 random하게 바꾸어 적용되도록 함. 그리고 50% 이상의 맞추는 것을 목표로 하고, 맞추는 비율에 따라 포인트가 부여됨
