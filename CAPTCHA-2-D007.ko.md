...
...
```
1. 위 홈페이지 로그인에 사용할 이메일 주소 "name@subdomain.domain.com"의 "name"은 실제 존재하는 이름, "subdomain"과 "domain"은 실제 존재하는 단어를 사용. 이름은 신문사 홈페이지에서 기자 이름들을 추출하여 사용. subdomain, domain 단어는 unix dictionary에서 있는 것을 random으로 선택하여 사용
1. 자동 생성된 숫자 CAPTCHA 이미지 내의 숫자 가운데 같은 크기,폰트,기울기 숫자는 두 개까지만 허용됨
1. 보너스 문제의 차량 번호판은 "12가3456"와 같은 2자리+한글+4자리 형식의 번호판으로 제한
