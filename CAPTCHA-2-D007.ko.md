...
```
이 영역은 콘텐츠를 만든 쪽에서 제공한 모든 학습 자료와 링크들
아래 online forum에 'resource'에 있는 내용 소개
```


### 프로젝트를 수행을 위한 online Forum
...
*** 에 따라.
학습 Resource 링크 (*** = resource)
 - 동기부여 용 스토리 동영상, 링크 등
 - 동영상, 오픈소스 코드, 블로그, 서적 (어떤 책, 몇 장, 주제)
 - 이 프로젝트를 수행하면서 체화해야 할 지식 그 자체가 쓰여진 책
   (정돈된 지식을 얻기 위한 리소스로서)
 - 이 프로젝트 관련 use case 동영상 (동기부여용)
이 프로젝트에 대한 토론 및 Q&A 링크 (*** = forum)
 - 이 프로젝트에 대한 모든 사람의 thread 토론과 QA
이 프로젝트에 대한 code review link (*** = codereview)
 - 이 프로젝트 관련 코드에 대한 code review 사이트
이 프로젝트에 대한 동료평가 링크 (*** = evaluation)
이 프로젝트에 대한 이전 멘토링 결과 (*** = mentor)
이 프로젝트를 같은 하고 있는 (했던) 사람들 (*** = peer)
...
* v007 :
 1. CAPTCHA의 숫자 이미지를 인공지능을 해결하는 Machine Learning 프로젝트 콘텐츠 (예시)
 1. 아래에 버전 history는 가상적으로 적은 것임
* v...
* v002 :
 1. 프로젝트 이미지 데이터 보강으로 프로젝트 완성도 기준 수치 조정 
* v001 :
 1. CAPTCHA의 숫자 이미지를 인공지능으로 풀어내는 Machine Learning 프로젝트 콘텐츠 설계
 1. 프로젝트 정의, 역량정의, 프로젝트 기술, 평가가이드, 데이터 파일 생성
